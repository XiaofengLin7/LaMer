...
def init_search_engine(num_products=None):
    ...
    print(f'[WebShop] init_search_engine: num_products={num_products} → index={indexes} (path={index_path})', flush=True)
...
def load_products(filepath, attrpath=None, num_products=None, human_goals=True):
    ...
    with open(attrpath if attrpath is not None else DEFAULT_ATTR_PATH) as f:
